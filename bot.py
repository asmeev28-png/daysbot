...
class BirthdayBot:
    ...
    async def _set_commands(self):
        ...
        commands = [
            ...
            BotCommand("list_events", "Список праздников"),
            BotCommand("next_events", "Ближайшие праздники"),
    ...
    async def _handle_list_events(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not events:
            await update.message.reply_text(
                "📅 Памятных дат пока нет.\n\n"
                "Чтобы добавить памятную дату/праздник (админы):\n"
                "`/add_event 01.05 Название праздника`\n"
        ...
        message += "`/add_event [дата] [название]` + текст - добавить праздник/памятную дату\n"
        message += "`/delete_event [ID]` - удалить событие\n"
        message += "`/toggle_event [ID]` - вкл/выкл событие\n"
        message += "`/next_events` - ближайшие праздники и памятные даты"
    ...
    async def _handle_next_events(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not events:
            await update.message.reply_text("📅 Ближайших праздников и памятных дат в чате нет.")
            return
        ...
        message = "📅 **Ближайшие праздники/памятные даты:**\n\n"
    ...
    async def _handle_add_event(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text("❌ Произошла ошибка при добавлении даты.")
    ...
    async def _handle_toggle_event(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user.id not in admin_ids and user.id not in Config.get_owners():
            await update.message.reply_text("❌ Только администраторы могут управлять памятными датами.")
            return
                ...
                if success:
                    ...
                    try:
                        welcome_message = (
                            ...
                            f"• `/list_events` - список праздников и памятных дат\n\n"
                            f"Администраторы чата могут:\n"
                            f"• Добавлять праздники и памятные даты\n"
...
• `/add_event [дата] [название]` + текст - добавить праздник/памятную дату
